...
when (ODIN_OS == .Darwin) {
  foreign import gio2 "../LIBS/libgio-2.0.dylib"
}  
...
when (ODIN_OS == .Darwin) {
    ...
    credentials_get_unix_pid :: proc(credentials: [^]Credentials, error: ^^glib.Error) -> u32 ---

    @(link_name = "g_credentials_get_unix_user")
    credentials_get_unix_user :: proc(credentials: [^]Credentials, error: ^^glib.Error) -> u32 ---

    @(link_name = "g_credentials_set_unix_user")
    credentials_set_unix_user :: proc(credentials: [^]Credentials, uid: u32, error: ^^glib.Error) -> glib.boolean ---
}

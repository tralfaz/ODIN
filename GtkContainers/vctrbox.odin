...
AppActivateCB :: proc "c" (app :^gtk.Application, user_data :glib.pointer) {
  ...
  vctrbox := gtk.center_box_new()
  gtk.orientable_set_orientation(cast(^gtk.Orientable)vctrbox,                                                   gtk.Orientation.VERTICAL)
  gtk.window_set_child(appwin, vctrbox)
    
  start := gtk.button_new_with_label("Start")
  gtk.center_box_set_start_widget(cast(^gtk.CenterBox)vctrbox, start)
    
  center := gtk.button_new_with_label("Center")
  gtk.center_box_set_center_widget(cast(^gtk.CenterBox)vctrbox, center)
//    gobj.signal_connect(button, "clicked",
//                       cast(gobj.Callback)on_button_clicked, appwin)
  gtk.widget_set_vexpand(center, true)

  end := gtk.button_new_with_label("End")
  gtk.center_box_set_end_widget(cast(^gtk.CenterBox)vctrbox, end)
